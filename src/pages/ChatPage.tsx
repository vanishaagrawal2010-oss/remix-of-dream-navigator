...
const ChatPage = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-57px)] md:h-screen">
      ...
      <div className="hidden lg:flex w-72 flex-col border-r border-border/50 bg-sidebar/50">
          ...
          {conversations.map(conv => (
            <button
              ...
              className={cn(
                ...
                conv.id === conversationId ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted/50"
              )}
            >
        ...
        {messages.length === 0 && !conversationId ? (
          <div className="flex-1 flex items-center justify-center p-8">
            <div className="text-center max-w-md space-y-4">
              <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-primary/10 glow-border">
                <Bot className="h-8 w-8 text-primary" />
              </div>
              <h2 className="font-heading text-2xl font-bold">AI University Counsellor</h2>
              <p className="text-muted-foreground text-sm">
                Ask me anything about universities, applications, scholarships, SOPs, or your study abroad plans. I know your profile and can give personalised advice.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-4">
                {["Which universities match my profile?", "Help me plan my application timeline", "What scholarships am I eligible for?", "Review my SOP draft"].map(q => (
                  <button
                    key={q}
                    onClick={() => { setInput(q); }}
                    className="text-left text-xs p-3 rounded-lg bg-muted/50 border border-border/50 text-muted-foreground hover:text-foreground hover:border-primary/30 transition-all"
                  >
                    {q}
                  </button>
                ))}
